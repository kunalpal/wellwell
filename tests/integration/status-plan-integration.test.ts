  ...
  async plan(ctx: any): Promise<any> {
    ...
    if (this.packageDependencies) {
      for (const dep of this.packageDependencies) {
        if (!dep.platforms || dep.platforms.includes(ctx.platform)) {
          const { addPackageContribution } = await import(
            "../../src/core/contrib.js"
          );
    ...
    if (this.customTemplate) {
      ...
      if (!exists) {
        ...
        try {
          const currentTheme =
            (ctx.state.get as any)("themes.current") || "default";
          const { themeContextProvider } = await import(
            "../../src/core/theme-context.js"
          );
...
describe("Plan-Status-Apply Integration", () => {
  ...
  beforeEach(() => {
    jest.clearAllMocks();
    Object.keys(mockFileContents).forEach(
      (key) => delete mockFileContents[key],
    );
  ...
  describe("Theme-aware configuration workflow", () => {
    it("should handle theme changes correctly", async () => {
      ...
      mockFileContents[themeConfigPath] =
  ...
  describe("Error handling in workflow", () => {
    ...
    it("should handle theme context errors gracefully", async () => {
      ...
      (themeContextProvider.getThemeColors as jest.Mock).mockRejectedValue(
        new Error("Theme context failed"),
      );
  ...
  describe("Robust status checking with state comparison", () => {
    it("should detect when file changes after apply (external modification)", async () => {
      ...
      expect(detailedStatuses["test:integration"].message).toContain(
        "planned change",
      );
    });
    ...
    it("should detect when expected state changes (theme change)", async () => {
      ...
      (themeContextProvider.getThemeColors as jest.Mock).mockResolvedValue({
        name: "nord",
      });
      ...
      expect(detailedStatuses["test:theme-change"].message).toContain(
        "planned change",
      );
    });
    ...
    it("should handle state comparison errors gracefully", async () => {
      ...
      errorConfig.captureState = jest
        .fn()
        .mockRejectedValue(new Error("State capture failed"));
    ...
    it("should detect staleness when plan changes but file content remains the same", async () => {
      const dynamicConfig = new TestIntegrationConfig({
        ...
        template: (ctx: any) => {
          ...
          const timestamp =
            (ctx.state.get as any)("dynamic.timestamp") || "initial";
