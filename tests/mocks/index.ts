...
export const createMockContext = (
  overrides: Partial<ConfigurationContext> = {},
): ConfigurationContext => ({
...
export const resetAllMocks = () => {
  ...
  Object.values(mockFs)
    ...
    .forEach((mock) => (mock as any).mockReset());
  mockExec.mockReset();
  mockSpawn.mockReset();
  Object.values(mockProcess)
    .filter((v) => typeof v === "function")
    .forEach((mock) => (mock as any).mockReset());
  Object.values(mockOs).forEach((mock) => mock.mockReset());
  Object.values(mockPath)
    .filter((v) => typeof v === "function")
    .forEach((mock) => (mock as any).mockReset());
...
export const mockCommandSuccess =
  (stdout = "", stderr = "") =>
  (mockExec: any) => {
    mockExec.mockResolvedValue({ stdout, stderr });
  };

export const mockCommandFailure =
  (error: Error | string = "Command failed") =>
  (mockExec: any) => {
