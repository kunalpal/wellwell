...
class PathsModule extends BaseModule {
  ...
  async plan(ctx: ConfigurationContext): Promise<PlanResult> {
    ...
    if (added > 0) {
      changes.push({
        ...
        impact: ['Shell environment will be updated', 'New binaries may be available'],
        riskLevel: 'low',
      });
    }
    ...
    if (pathOrderChanged) {
      ...
      const reorderedPaths = resolved.filter((p, i) => prev[i] && prev[i] !== p);
      
      let details = 'PATH order changes:';
      if (addedPaths.length > 0) details += `\n  + Added: ${addedPaths.join(', ')}`;
      if (removedPaths.length > 0) details += `\n  - Removed: ${removedPaths.join(', ')}`;
      if (reorderedPaths.length > 0) details += `\n  ~ Reordered: ${reorderedPaths.length} paths`;
  ...
  async apply(ctx: ConfigurationContext): Promise<ApplyResult> {
    return this.safeExecute(ctx, 'PATH resolution', async () => {
      ...
      return this.createSuccessResult(true, `Resolved ${resolved.length} PATH entries`);
  ...
  async status(ctx: ConfigurationContext): Promise<StatusResult> {
    try {
      ...
      if (pathValidation.missing.length > 0) {
        issues.push(`${pathValidation.missing.length} paths don't exist: ${pathValidation.missing.slice(0, 3).join(', ')}${pathValidation.missing.length > 3 ? '...' : ''}`);
        recommendations.push('Review path contributions for invalid directories');
      }
      
      if (pathValidation.inaccessible.length > 0) {
        issues.push(`${pathValidation.inaccessible.length} paths not accessible: ${pathValidation.inaccessible.slice(0, 3).join(', ')}${pathValidation.inaccessible.length > 3 ? '...' : ''}`);
      ...
      return this.createStatusResult(status, `Managing ${resolved.length} paths`, {
      ...
      return this.createStatusResult('failed', 'Error checking PATH status', {
        ...
        recommendations: ['Check logs for details', 'Verify file system access'],
      });
    }
  }
