...
export const zshrcPluginsModule: ConfigurationModule = {
  ...
  description:
    "Configure zsh plugins with zinit (autosuggestions, syntax-highlighting)",
  ...
  async plan(ctx): Promise<PlanResult> {
    ...
    try {
      ...
      if (!hasPluginsBlock) {
        ...
        const endIdx = content.indexOf(ZSHRC_PLUGINS_MARKER_END);
        if (startIdx !== -1 && endIdx !== -1) {
          const currentBlock = content.substring(
            startIdx,
            endIdx + ZSHRC_PLUGINS_MARKER_END.length,
          );
        ...
        if (hasPluginsBlock) {
          ...
          if (startIdx !== -1 && endIdx !== -1) {
            ...
            if (currentBlock === expectedBlock.trim()) {
              return {
                status: "applied",
                message: `Zinit configured with ${DEFAULT_PLUGINS.length} plugins`,
              };
            } else {
              return {
                status: "stale",
                message: "Zsh plugins configuration needs update",
              };
            }
          }
          return {
            status: "stale",
            message: "Zsh plugins configuration corrupted",
          };
        } else {
          return {
            status: "stale",
            message: "Plugins not configured in zshrc",
          };
        }
      } catch {
