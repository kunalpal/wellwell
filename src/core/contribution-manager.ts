...
export class GenericContributionManager<T = any> implements ContributionManager<T> {
  ...
  add(ctx: ConfigurationContext, contribution: Contribution<T>): boolean {
    ...
    const exists = current.some(c => this.idExtractor(c.data) === this.idExtractor(contribution.data));
  ...
  resolve(ctx: ConfigurationContext): T[] {
    ...
    const filtered = contribs.filter(c => 
      !c.platforms || c.platforms.includes(ctx.platform)
    );
...
export class PathContributionManager extends GenericContributionManager<string> {
  ...
  resolve(ctx: ConfigurationContext): string[] {
    const contribs = ctx.state.get<Contribution<string>[]>('contrib.paths') ?? [];
    const prepend = contribs.filter(c => c.data.startsWith('prepend:')).map(c => c.data.slice(8));
    const append = contribs.filter(c => !c.data.startsWith('prepend:')).map(c => c.data);
...
export class AliasContributionManager extends GenericContributionManager<{ name: string; value: string }> {
  constructor() {
    super(
      'contrib.aliases',
      'resolved.aliases',
  ...
  resolve(ctx: ConfigurationContext): { name: string; value: string }[] {
    const contribs = ctx.state.get<Contribution<{ name: string; value: string }>[]>('contrib.aliases') ?? [];
