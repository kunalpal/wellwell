...
export abstract class PackageManager extends BaseModule {
  ...
  async isApplicable(ctx: ConfigurationContext): Promise<boolean> {
    return (
      this.config.platforms.includes(ctx.platform) && (await this.isAvailable())
    );
  }
  ...
  protected async updatePackageCache(): Promise<void> {
    if (this.config.updateCommand) {
      const command = this.config.requiresSudo
        ? `sudo ${this.config.updateCommand}`
  ...
  async plan(ctx: ConfigurationContext): Promise<PlanResult> {
    ...
    const packageKey =
      this.config.command === "homebrew" ? "brew" : this.config.command;
    ...
    if (packages.length > 0) {
      ...
      if (toInstall.length > 0) {
        const updateText = this.config.updateCommand
          ? "Update package cache and "
          : "";
        changes.push({
    ...
    try {
      ...
      if (packages.length > 0) {
        ...
        if (toInstall.length > 0) {
          this.logProgress(
            ctx,
            `Installing ${toInstall.length} ${this.config.name} packages`,
          );
          ...
          if (result.failed.length > 0) {
            this.logError(
              ctx,
              new Error(`Failed to install: ${result.failed.join(", ")}`),
            );
          }
      ...
      return this.createSuccessResult(
        false,
        `${this.config.name} packages up to date`,
      );
    } catch (error) {
    ...
    try {
      ...
      if (!isAvailable) {
        return this.createStatusResult(
          "stale",
          `${this.config.name} not available`,
          {
            issues: [
      ...
      if (packages.length === 0) {
        return this.createStatusResult(
          "applied",
          `${this.config.name} available, no packages configured`,
          {
            metadata: {
      ...
      const packageValidation = await this.validatePackages(
        packages,
        installed,
      );
      ...
      if (missing.length > 0) {
        ...
        issues.push(
          `${missing.length} packages missing: ${missingNames.slice(0, 5).join(", ")}${missing.length > 5 ? "..." : ""}`,
        );
      ...
      if (outdated.length > 0) {
        ...
        issues.push(
          `${outdated.length} packages outdated: ${outdatedNames.slice(0, 5).join(", ")}${outdated.length > 5 ? "..." : ""}`,
        );
  ...
  private async validatePackages(
    ...
    if (duplicates.length > 0) {
      issues.push(
        `Duplicate packages configured: ${[...new Set(duplicates)].join(", ")}`,
      );
      recommendations.push(
        "Remove duplicate package entries from configuration",
      );
    }

    // Check for packages with missing names
    const packagesWithoutNames = packages.filter(
    ...
    const incompatiblePackages = packages.filter(
      (p) =>
        ...
        !p.platforms.some((platform: Platform) =>
          this.config.platforms.includes(platform),
        ),
    );
    if (incompatiblePackages.length > 0) {
      issues.push(
        `${incompatiblePackages.length} packages not compatible with current platforms`,
      );
