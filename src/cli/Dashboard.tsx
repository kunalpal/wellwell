...
async function getModuleDetails(
  ...
  try {
    if (module.getDetails) {
      ...
      return [`Module: ${module.id}`, "  No specific details available"];
    }
  } catch (error) {
...
export default function Dashboard({ verbose }: DashboardProps) {
  const { exit } = useApp();
  const { currentTheme, themeColors, switchTheme, getAvailableThemes } =
    useTheme();
    ...
    const loadModuleInfo = async () => {
      try {
        ...
        if (module) {
          ...
          if (
            !planCache.current[selectedModule.id] &&
    ...
    <Box flexDirection="column">
      <Box>
        <Text>
          ...
          {chalk.gray(
            "↑/↓/j/k: navigate  a: apply  A: apply all  p: plan  s: refresh  tab: switch theme  q: quit",
          )}
        </Text>
      </Box>
      <Box>
      ...
      <Box marginTop={1} flexDirection="column">
        ...
        <Box flexDirection="column" flexShrink={1}>
          {sorted.map((r, idx) => {
            ...
            const isHighlighted =
              selectedModule &&
              (r.id === selectedModule.id || downstreamDeps.has(r.id));
                          ...
                          .map(
                            (depId, depIdx) =>
                              ...
                              formatDependency(
                                depId,
                                rows[depId]?.status,
                                !isModuleApplicable(depId, rows),
                                downstreamDeps.has(depId),
                                themeColors,
                              ),
                          )
                          .join("")
          ...
          <Box marginTop={1} flexDirection="column">
            ...
            {selectedModule.status !== "applied" && (
              <>
                <Box marginTop={1}>
                  <Text bold>
                    {getThemeColors(themeColors).semantic.warning("PLAN")}
                  </Text>
                  {planLoading && (
                    <Text>
                      {getThemeColors(themeColors).semantic.warning(" ")}
...
function formatModuleNameWithSelection(
  moduleId: string,
  isSelected: boolean,
